      ...
      <div className="container-fluid px-4 px-lg-5">
        ...
        <div className={`collapse navbar-collapse ${isNavOpen ? 'show' : ''}`} id="navbarSupportedContent">
          <ul className="navbar-nav me-auto mb-2 mb-lg-0 ms-lg-4">
            <li className="nav-item"><a className="nav-link" href="#!">{t('nav.tournaments')}</a></li>
            <li className="nav-item"><a className="nav-link" href="#!">{t('nav.ranking')}</a></li>
            <li className="nav-item"><a className="nav-link" href="#!">{t('nav.news')}</a></li>
          </ul>
          <div className="d-flex">
            {auth && auth.accessToken ? ( // Check if the user is authenticated
              <button className="btn btn-outline-dark" type="button" onClick={handleLogoutClick}>{t('nav.logout')}</button>
            ) : (
              <button className="btn btn-outline-dark" type="button" onClick={handleLoginClick}>{t('nav.login')}</button>
            )}
          </div>
        </div>
      </div>
