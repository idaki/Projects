...
export async function fetchWithSettingsForMetadata(url, options) {
    const { token, header } = getCsrfTokenFromMeta();

    const defaultHeaders = {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${fetchCsrfToken()}`,
        [header]: token  // Use the header name dynamically
    };
    ...
    try {
        ...
        // Add specific logic for handling metadata if needed here
        // For example, you might want to extract certain headers or other response metadata
